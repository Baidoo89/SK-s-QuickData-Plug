...
export function StoreLinkCard({ storePath }: StoreLinkCardProps) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Shop link copied", description: "Ready to share." })
    } catch {
    ...
    <Card className="premium-surface overflow-hidden rounded-lg">
      <CardHeader className="border-b border-border/70 bg-muted/20 pb-3">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <CardTitle className="text-sm font-semibold">Shop link</CardTitle>
            <CardDescription className="text-xs">Share this with customers.</CardDescription>
          </div>
