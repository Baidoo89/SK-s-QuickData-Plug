              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="truncate text-base font-semibold leading-tight">{order.phoneNumber || "N/A"}</p>
                      <p className="truncate text-xs font-medium text-muted-foreground">{order.bundle || "Bundle N/A"}</p>
                      <p className="font-mono text-[10px] text-muted-foreground">#{order.id.slice(-8)} | {formatDate(order.createdAt)}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="font-semibold leading-tight">{formatGhanaCedis(order.total)}</p>
                      <Badge variant="outline" className="mt-1 px-2 py-0 text-[10px]">{order.network || "-"}</Badge>
                    </div>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                    ...
                    <div>
                      <p className="truncate font-medium text-foreground">{order.sourceLabel}</p>
                      <p>Source</p>
                    </div>
                    <div>
                      <p className="truncate font-medium text-foreground">{order.paymentOwner} / {order.paymentStatus}</p>
                      <p>Payment</p>
                    </div>
                    <div>
                      <p className="truncate font-medium text-foreground">{order.fulfillmentMode}</p>
                      <p>Fulfillment</p>
                    </div>
                    <div>
                      <p className={order.profit > 0 ? "font-semibold text-primary" : "font-medium text-muted-foreground"}>{formatGhanaCedis(order.profit)}</p>
                      <p>Profit</p>
                    </div>
                    <div>
                      <p className="truncate font-medium text-foreground">{order.sellerRole}</p>
                      <p>Seller</p>
                    </div>
                  </div>
                    ...
                    {editableStatus ? (
                      ...
                      <Badge variant={statusTone(order.status)} className="justify-center py-1.5">{order.status}</Badge>
                    )}
                  </div>
                </div>
              </div>
