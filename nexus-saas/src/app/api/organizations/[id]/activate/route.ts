...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      const updatedOrg = await tx.organization.update({ where: { id: orgId }, data: { active: true } })
      const prodRes = await tx.product.updateMany({ where: { organizationId: orgId }, data: { active: true } })
      ...
      const audit = await tx.auditLog.create({
        data: {
          ...
          after: JSON.stringify({ orgActive: true }),
    ...
    console.error("[ORG_ACTIVATE]", error)
