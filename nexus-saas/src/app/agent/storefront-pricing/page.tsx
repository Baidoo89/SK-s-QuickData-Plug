...
export default function AgentStorefrontPricingPage() {
  ...
  async function loadPrices() {
    const res = await fetch("/api/agent/storefront-prices")
  ...
  const networks = useMemo(
    () => Array.from(new Set(products.map((product) => product.provider).filter(Boolean))).sort(),
    [products],
  )
  ...
  async function savePrice(product: ProductRow, value: number) {
    ...
    try {
      ...
      setProducts((rows) =>
        rows.map((row) =>
          row.id === product.id ? { ...row, storefrontPrice: value, profit: Math.max(value - row.buyPrice, 0) } : row,
        ),
      )
    ...
    <div className="portal-page space-y-6">
      ...
      <Card className="premium-surface overflow-hidden rounded-lg">
        ...
        <CardContent className="space-y-3">
          ...
          {savingId && <p className="text-xs text-muted-foreground">Saving price...</p>}
        </CardContent>
      </Card>
    </div>
  )
