...
async function resolveAgentId(user: { id: string; name: string | null; agentId: string | null; organizationId: string }) {
  ...
  await db.user.update({ where: { id: user.id }, data: { agentId: fallbackAgent.id } })
  return fallbackAgent.id
}

async function getAgentServiceRequests(status?: string, serviceId?: string) {
  const session = await auth()
  if (!session?.user?.email) return { rows: [], accessError: "LOGIN" as const }
  ...
  const userIds = Array.from(new Set(requests.map((request) => request.userId || request.sellerUserId).filter(Boolean))) as string[]
  ...
  const rows = requests.map((request): ServiceRequestRow => {
    const details = parseDetails(request.details)
    const product = productMap.get(request.productId)
    const sellerId = request.userId || request.sellerUserId
    return {
      id: request.id,
      ...
      sellerRole: request.sellerRole || "AGENT",
      sellerName: sellerId ? sellerMap.get(sellerId) || "Reseller" : "Your storefront",
  ...
  return { rows, accessError: null as null }
}

export default async function AgentServiceRequestsPage({
  searchParams,
}: {
  ...
  const [{ rows, accessError }, allRowsResult] = await Promise.all([
    getAgentServiceRequests(status, serviceId),
    getAgentServiceRequests(status),
  ])

  if (accessError === "LOGIN") {
    return <PortalAccessMessage title="Login required" description="Sign in with an approved agent account to view service requests." />
  }

  if (accessError === "PROFILE") {
    return <PortalAccessMessage title="Agent profile unavailable" description="This account is not linked to an approved agent profile." />
  }

  const allRowsForStatus = allRowsResult.rows
  ...
  const queryFor = (nextServiceId?: string) => {
    ...
    return query ? `/agent/service-requests?${query}` : "/agent/service-requests"
  }
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="max-w-xl text-sm text-muted-foreground">
            View registration and service requests submitted through your agent storefront and reseller links.
          </p>
        </div>
        ...
        <CardContent className="space-y-3 p-4">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">View each service separately across your agent and reseller storefront activity.</p>
            </div>
        ...
        <CardContent className="p-3 sm:p-6">
          {rows.length === 0 ? (
            <EmptyState
              ...
              action={hasFilters ? { label: "Show All", href: "/agent/service-requests" } : { label: "Share Storefront", href: "/agent/storefronts" }}
            />
          ) : (
