...
export default function AgentBuyBulkPage() {
  ...
  async function validateBulkOrders() {
    try {
      ...
      const res = await fetch("/api/agent/validate-bulk-orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          phones,
                ...
                {bulkResults.map((result, idx) => (
                  <div key={idx} className="rounded-lg border bg-background p-3 shadow-sm text-xs">
                    ...
                    <div className="mt-2 flex items-center justify-between gap-3">
                      ...
                      <span className="font-medium">{formatGhanaCedis(Number(result.amount || 0))}</span>
                    </div>
                  ...
                  <tbody>
                    {bulkResults.map((result, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="border p-2 text-right">{formatGhanaCedis(Number(result.amount || 0))}</td>
